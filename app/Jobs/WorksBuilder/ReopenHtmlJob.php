...
use App\Services\WorksBuilder\Ai\PromptBuilders\ReopenPromptBuilder;
...
/**
 * 명세 v11 §1.4.2 — 재실행 Task용 HTML 생성 Job.
 */
class ReopenHtmlJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        try {
            ...
            if ($task->parent_task_id) {
                $sourceHtmlId = GeneratedHtml::where('task_id', $task->parent_task_id)
                    ->orderByDesc('version')
                    ->value('id');
            }

            DB::transaction(function () use ($task, $result, $log, $html, $sourceHtmlId) {
            ...
            Log::error('[WB] ReopenHtmlJob: AI 호출 실패', [
                'task_id' => $task->id,
                'status'  => $e->statusCode,
            ]);
            $task->update([
            ...
            Log::error('[WB] ReopenHtmlJob: 예외', ['task_id' => $task->id, 'error' => $e->getMessage()]);
            $task->update([
                'status'        => Task::STATUS_IN_PROGRESS,
                'current_stage' => $task->mode === 'enhance' ? 'spec_review' : 'option_input',
            ]);
