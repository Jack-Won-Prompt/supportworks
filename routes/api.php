...
/*
|--------------------------------------------------------------------------
| AI Works — 운영 사이트 오류 수집
|--------------------------------------------------------------------------
| 다섯 개의 운영 사이트가 예외를 만나면 여기로 보낸다. 데몬 API 와 인증이
| 다르다 — 작업 PC 가 아니라 운영 사이트의 자격(aiw_error_sources)이다.
|
| 어느 프로젝트인지는 토큰으로 정한다. 본문의 project_id 는 읽지 않는다.
| aiw.agent 미들웨어를 붙이지 않는 이유도 그것이다.
*/
Route::post('aiw/errors', [\App\Http\Controllers\Api\AiWork\ErrorIntakeController::class, 'store'])
    ->name('api.aiw.errors.store');
...
Route::prefix('aiw')->middleware('aiw.agent')->name('api.aiw.')->group(function () {
    ...
    // 경로·git·브랜치·작업트리 점검 결과. 지시 전에 무엇을 고쳐야 하는지 화면에 띄운다.
    Route::post('mappings/setup',    [\App\Http\Controllers\Api\AiWork\DaemonController::class, 'reportSetup'])->name('mappings.setup');
    // 운영 서버가 따로인 프로젝트는 담당자 PC 가 배포하고 결과를 여기로 보낸다.
