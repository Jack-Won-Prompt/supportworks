...
<div class="admin-wrap">
    ...
    <aside class="admin-side">
        ...
        <nav class="admin-nav">
            ...
            <a href="{{ route('admin.maintenances.index') }}" class="{{ request()->routeIs('admin.maintenances.*') ? 'active' : '' }}">
                ...
                {{ __('admin.sr_tickets') }}
                @php
                    $pendingMaint = \App\Models\ProjectMaintenance::where('status','pending')->count();
                @endphp
                @if($pendingMaint > 0)
                <span class="nav-badge">{{ $pendingMaint }}</span>
                @endif
            </a>
...
(function() {
    const MENU = [
        ...
        { label:'{{ __("admin.sr_tickets") }}',       url:'{{ route("admin.maintenances.index") }}',  icon:'🔧' },
