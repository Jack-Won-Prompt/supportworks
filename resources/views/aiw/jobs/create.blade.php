        ...
        <form method="POST" action="{{ route('projects.ai-works.store', $project) }}"
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <div x-data="{ noLimit: @js((bool) $noLimitDefault) }">
                    ...
                    {{-- 시간 제한은 설정에만 있고 실제로 걸려 있지 않다(JOB_TIMEOUT_SEC
                         등이 코드에서 쓰이지 않는다). 있지도 않은 방어선을 안내하면
                         사람이 안심하고 상한을 꺼 버린다. 사실대로 적는다. --}}
                    <p class="mt-1 text-xs text-amber-700" x-show="noLimit" x-cloak>
                        <span class="font-semibold">자동으로 멈추지 않습니다.</span>
                        지금은 이 상한이 유일한 자동 중단 장치입니다 — 끄면 사람이 취소할 때까지 계속 돕니다.
                    </p>
                </div>
            </div>
