...
<a href="{{ route('meeting-minutes.index') }}" style="font-size:13px;color:#94a3b8;text-decoration:none;"
...
<div style="display:flex;gap:8px;">
    <a href="{{ route('meeting-minutes.download', $meetingMinute) }}"
       style="display:inline-flex;align-items:center;gap:5px;padding:7px 14px;border:1.5px solid #ddd6fe;border-radius:8px;font-size:13px;font-weight:600;color:#7c3aed;text-decoration:none;background:#faf5ff;">
    ...
    <a href="{{ route('meeting-minutes.edit', $meetingMinute) }}"
       style="display:inline-flex;align-items:center;gap:5px;padding:7px 14px;border:1.5px solid #e8e3ff;border-radius:8px;font-size:13px;font-weight:600;color:#64748b;text-decoration:none;background:#fff;">
    ...
    <form method="POST" action="{{ route('meeting-minutes.destroy', $meetingMinute) }}"
        ...
        @csrf @method('DELETE')
        <button type="submit"
                style="display:inline-flex;align-items:center;gap:5px;padding:7px 14px;border:1.5px solid #fecaca;border-radius:8px;font-size:13px;font-weight:600;color:#ef4444;background:#fff;cursor:pointer;">
...
<div style="max-width:1100px;margin:0 auto;padding:24px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 300px;gap:20px;align-items:start;">
        ...
        <div>
            ...
            <div style="background:#fff;border:1px solid #f0eeff;border-radius:14px;padding:22px;margin-bottom:16px;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;flex-wrap:wrap;">
                    ...
                    <span style="font-size:12px;color:#7c3aed;background:#f5f3ff;padding:3px 10px;border-radius:6px;">{{ $meetingMinute->project->name }}</span>
                    @endif
                </div>
                <h2 style="font-size:20px;font-weight:800;color:#1e1b2e;margin-bottom:16px;">{{ $meetingMinute->title }}</h2>
                ...
                <div style="margin-top:14px;padding-top:14px;border-top:1px solid #f0eeff;">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:6px;">
                        ...
                        <span style="display:inline-flex;align-items:center;gap:5px;padding:4px 10px;background:var(--t50);border:1px solid var(--t200);border-radius:20px;font-size:12px;font-weight:600;color:var(--t700);">
            ...
            <div style="background:#fff;border:1px solid #f0eeff;border-radius:14px;padding:22px;margin-bottom:16px;">
                <div style="font-size:13px;font-weight:700;color:#1e1b2e;margin-bottom:12px;display:flex;align-items:center;gap:6px;">
            ...
            <div style="background:linear-gradient(135deg,#f5f3ff,#ede9fe);border:1px solid #c4b5fd;border-radius:14px;padding:22px;margin-bottom:16px;">
                <div style="font-size:13px;font-weight:700;color:#7c3aed;margin-bottom:12px;display:flex;align-items:center;gap:6px;">
            ...
            <div style="background:#fff;border:1px solid #f0eeff;border-radius:14px;padding:22px;margin-bottom:16px;">
                ...
                <div style="border-top:1px solid #f0eeff;padding-top:14px;margin-top:14px;">
                ...
                @empty
                <div style="font-size:13px;color:#94a3b8;text-align:center;padding:20px 0;">{{ __('maintenance.memo_empty') }}</div>
                @endforelse
            </div>
        </div>
        ...
        {{-- 사이드: Action Items --}}
        <div>
            <div style="background:#fff;border:1px solid #f0eeff;border-radius:14px;padding:18px;position:sticky;top:72px;">
                ...
                <div style="margin-bottom:14px;">
                    <div style="font-size:11px;font-weight:700;color:#94a3b8;letter-spacing:.06em;text-transform:uppercase;margin-bottom:8px;">
                        {{ $statusLabel }} ({{ $items->count() }})
                    </div>
                    @foreach($items as $item)
                    <div style="border:1px solid #f0eeff;border-radius:10px;padding:12px;margin-bottom:7px;{{ $item->isOverdue() ? 'border-color:#fecaca;' : '' }}">
                        ...
                        <div style="display:flex;gap:4px;margin-top:8px;">
                            ...
                            <form method="POST" action="{{ route('meeting-minutes.action-items.destroy', $item) }}"
                                ...
                                @csrf @method('DELETE')
                                <button type="submit" style="font-size:10px;padding:3px 7px;border:1px solid #fecaca;border-radius:5px;background:#fff;color:#ef4444;cursor:pointer;">{{ __('common.delete') }}</button>
                            </form>
                        </div>
                    </div>
                    @endforeach
                ...
                <div style="font-size:13px;color:#94a3b8;text-align:center;padding:20px 0;">{{ __('maintenance.action_item_empty') }}</div>
                @endif
            </div>
        </div>
    </div>
...
<div id="ai-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:10000;align-items:center;justify-content:center;"
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;width:100%;max-width:480px;box-shadow:0 20px 60px rgba(0,0,0,.2);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
            <span style="font-size:15px;font-weight:700;color:#1e1b2e;">{{ __('maintenance.action_item_add') }}</span>
