...
{{ __('maintenance.meeting_minutes') }}
@endsection
            ...
            <div style="background:#fff;border:1px solid #f0eeff;border-radius:14px;padding:18px;margin-bottom:14px;">
                ...
                <form method="POST" action="{{ route('meeting-minutes.memos.store', $meetingMinute) }}" style="margin-bottom:16px;">
                    ...
                    <div style="text-align:right;margin-top:6px;">
                        <button type="submit"
                                style="padding:6px 14px;background:var(--t600);color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;">
        ...
        <div>
            <div style="background:#fff;border:1px solid #f0eeff;border-radius:14px;padding:16px;position:sticky;top:0;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
                    ...
                    <button onclick="document.getElementById('ai-modal-popup').style.display='flex'"
                            style="font-size:11px;font-weight:700;padding:4px 9px;background:var(--t600);color:#fff;border:none;border-radius:7px;cursor:pointer;">
                        + {{ __('common.add') }}
                    </button>
                </div>
                    ...
                    <div style="border:1px solid #f0eeff;border-radius:10px;padding:10px;margin-bottom:6px;{{ $item->isOverdue() ? 'border-color:#fecaca;' : '' }}">
                        ...
                        <div style="display:flex;gap:3px;margin-top:7px;flex-wrap:wrap;">
                            ...
                            <form method="POST" action="{{ route('meeting-minutes.action-items.status', $item) }}">
                                ...
                                <button type="submit"
                                        style="font-size:10px;padding:2px 6px;border:1px solid #e8e3ff;border-radius:5px;background:#fff;color:#64748b;cursor:pointer;">
                                    {{ $sl }}
                                </button>
                            </form>
                            @endif
...
@section('modals')
{{-- Action Item 추가 모달 --}}
<div id="ai-modal-popup" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:10000;align-items:center;justify-content:center;"
    ...
    <div style="background:#fff;border-radius:16px;padding:22px;width:100%;max-width:460px;box-shadow:0 20px 60px rgba(0,0,0,.2);margin:16px;">
        ...
        <form method="POST" action="{{ route('meeting-minutes.action-items.store', $meetingMinute) }}">
            @csrf
            @php $minp = 'width:100%;padding:7px 11px;border:1.5px solid #e8e3ff;border-radius:8px;font-size:13px;color:#1e1b2e;outline:none;background:#fff;font-family:inherit;margin-bottom:9px;box-sizing:border-box;'; @endphp
            ...
            <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:4px;">
                <button type="button" onclick="document.getElementById('ai-modal-popup').style.display='none'"
                        style="padding:7px 14px;border:1.5px solid #e8e3ff;border-radius:8px;background:#fff;font-size:13px;color:#64748b;cursor:pointer;">{{ __('common.cancel') }}</button>
                <button type="submit"
                        style="padding:7px 18px;background:var(--t600);color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;">{{ __('common.add') }}</button>
            </div>
        </form>
    </div>
</div>
...
async function deleteMinute() {
    if (!confirm('{{ addslashes(__('maintenance.confirm_delete_meeting')) }}')) return;

    const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
    try {
        await fetch('{{ route('meeting-minutes.destroy', $meetingMinute) }}', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': token, 'Content-Type': 'application/x-www-form-urlencoded' },
            body: '_method=DELETE',
            redirect: 'manual',
        });
        if (window.parent && window.parent.closeMinutePopup) {
            window.parent.closeMinutePopup(true);
        } else {
