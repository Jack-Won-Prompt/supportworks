...
@section('title', '디자인 단계 승인 — 웍스 Agent')
...
.apv-progress-label { display:flex; justify-content:space-between; font-size:12px; color:#64748b; margin-bottom:6px; font-weight:600; }
...
.apv-item-note  { font-size:11.5px; color:#64748b; }
...
<div class="apv-header">
    <div class="apv-header-left">
        <h1>디자인 단계 승인 게이트</h1>
        <p>디자인 단계 산출물이 완료되면 매니저에게 승인을 요청하고 개발 준비 단계를 활성화합니다.</p>
    </div>
...
<div class="apv-approved-banner">
    ...
    <div>
        <div class="apv-approved-banner-text">디자인 단계 승인 완료</div>
        <div class="apv-approved-banner-sub">개발 준비 단계가 활성화되었습니다. 다음 단계로 진행하세요.</div>
    </div>
</div>
@endif
    ...
    <div>
        ...
        <div class="apv-section" id="diagnosisPanel">
            ...
            <div class="apv-list" id="blockingList">
                ...
                @php
                    $link = match($item['type']) {
                        'design_tokens'    => route('ai-agent.projects.design.tokens', $project),
                        'component_spec'   => route('ai-agent.projects.design.components', $project),
                        'layout_spec'      => route('ai-agent.projects.design.layout', $project),
                        'design_system_doc'=> route('ai-agent.projects.design.system', $project),
                        'dev_handoff'      => route('ai-agent.projects.design.figma-dev', $project),
                        default            => null,
                    };
                @endphp
            ...
            <div class="apv-list" id="warningList">
                ...
                @php
                    $link = match($item['type']) {
                        'screen_mapping' => route('ai-agent.projects.design.screens', $project),
                        'design_review'  => route('ai-agent.projects.design.validation', $project),
                        default          => null,
                    };
                    $cls  = $item['complete'] ? 'warn-ok' : 'warn-no';
                    $isPct = $item['type'] === 'screen_mapping';
                @endphp
    ...
    <div>
        ...
        <div class="apv-section" style="margin-top:16px;">
            ...
            <div style="display:flex;flex-direction:column;gap:8px;font-size:13px;color:#475569;">
                ...
                <div style="display:flex;justify-content:space-between;">
                    <span style="color:#94a3b8;font-size:12px;">Figma 매핑</span>
                    <span style="font-weight:700;">{{ $diagnosis['mapped_screens'] }} / {{ $diagnosis['total_screens'] }}개</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="apv-section" style="margin-top:16px;background:#f0fdf4;border-color:#bbf7d0;">
            ...
            <p style="font-size:12.5px;color:#166534;margin:0 0 12px;line-height:1.6;">
                개발 준비 단계가 활성화되었습니다. ERD, API 명세서, 권한 모델(RBAC) 작성을 시작할 수 있습니다.
            </p>
            <a href="{{ route('ai-agent.projects.pre-dev.index', $project) }}"
...
async function refreshDiagnosis(btn) {
    ...
    try {
        const res  = await fetch('{{ route('ai-agent.projects.design.approval.diagnosis', $project) }}', {
...
function updateDiagnosis(data) {
    ...
    if (warningList) {
        ...
        warningList.innerHTML = data.warnings.map(item => {
            ...
            const isPct = item.type === 'screen_mapping';
            let pill = '';
            if (item.coverage !== null) {
                const pillCls = item.complete ? 'ok' : 'low';
                pill = isPct
                    ? `<span class="apv-pill ${pillCls}">${item.covered}/${item.total} (${item.coverage}%)</span>`
                    : `<span class="apv-pill ${pillCls}">${item.coverage}점</span>`;
            }
            return `<div class="apv-item ${cls}">
                <span class="apv-item-icon">${icon}</span>
                <div class="apv-item-body">
                    <div class="apv-item-label">${item.label}${pill}</div>
                    ${item.note ? `<div class="apv-item-note">${item.note}</div>` : ''}
