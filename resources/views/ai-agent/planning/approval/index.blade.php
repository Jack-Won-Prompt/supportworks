...
@section('title', '기획 단계 승인 — 웍스 Agent')
...
<div class="apv-header">
    <div class="apv-header-left">
        <h1>기획 단계 승인 게이트</h1>
        <p>기획 단계 산출물이 완료되면 매니저에게 승인을 요청하고 디자인 단계를 활성화합니다.</p>
    </div>
...
<div class="apv-approved-banner">
    ...
    <div>
        <div class="apv-approved-banner-text">기획 단계 승인 완료</div>
        <div class="apv-approved-banner-sub">디자인 단계가 활성화되었습니다. 다음 단계로 진행하세요.</div>
    </div>
</div>
@endif
    ...
    <div>
        ...
        <div class="apv-section" id="diagnosisPanel">
            ...
            <div class="apv-list" id="blockingList">
                ...
                @php
                    $link = match($item['type']) {
                        'as_is_analysis'     => route('ai-agent.projects.planning.as-is', $project),
                        'to_be_requirements' => route('ai-agent.projects.planning.to-be', $project),
                        'gap_analysis'       => route('ai-agent.projects.planning.gap', $project),
                        'planning_doc'       => route('ai-agent.projects.planning.document', $project),
                        default              => null,
                    };
                @endphp
            ...
            <div class="apv-list" id="warningList">
                ...
                @php
                    $link = match($item['type']) {
                        'ia_flow'        => route('ai-agent.projects.planning.ia', $project),
                        'screen_prompts' => route('ai-agent.projects.planning.prompts', $project),
                        'mockup'         => route('ai-agent.projects.planning.mockups', $project),
                        default          => null,
                    };
...
async function refreshDiagnosis(btn) {
    ...
    try {
        const res  = await fetch('{{ route('ai-agent.projects.planning.approval.diagnosis', $project) }}', {
...
function updateDiagnosis(data) {
    ...
    if (warningList) {
        warningList.innerHTML = data.warnings.map(item => {
            ...
            const pill = item.coverage !== null
                ? `<span class="apv-pill ${item.complete ? 'ok' : 'low'}">${item.covered}/${item.total} (${item.coverage}%)</span>`
                : '';
            return `<div class="apv-item ${cls}">
                <span class="apv-item-icon">${icon}</span>
                <div class="apv-item-body">
