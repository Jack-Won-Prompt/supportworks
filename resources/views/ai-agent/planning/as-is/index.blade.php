...
.asis-empty { text-align:center; padding:40px 20px; color:#94a3b8; }
.asis-empty-icon { font-size:40px; margin-bottom:10px; }
.asis-empty-text { font-size:14px; font-weight:600; color:#64748b; margin-bottom:4px; }
.asis-empty-hint { font-size:12.5px; color:#94a3b8; }
...
<div class="asis-header">
    <div class="asis-header-left">
        <h1>AS-IS 분석 <span style="font-size:14px;font-weight:500;color:#94a3b8;">{{ $scopeLabel }}</span></h1>
    ...
    <div class="asis-header-right">
        ...
        />
        <a href="{{ $exportUrl }}" x-show="result !== null" x-cloak
           class="asis-btn secondary" title="Markdown 내보내기">
...
{{-- ── Polling indicator (shown when files still parsing) ──────────────── --}}
@if($files->whereIn('parse_status',['pending','parsing'])->count() > 0)
<div class="asis-polling-bar" id="polling-bar">
...
<div class="asis-section">
    ...
    <form method="POST" action="{{ route('ai-agent.projects.planning.as-is.upload', $project) }}"
...
<div class="asis-section">
    ...
    <div class="asis-file-list">
        ...
        <div class="asis-file-item">
            ...
            <form method="POST" action="{{ route('ai-agent.projects.planning.as-is.file.delete', [$project, $file]) }}"
...
<div class="asis-section">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;flex-wrap:wrap;">
        <button @click="triggerAnalyze()" class="asis-btn primary"
                :disabled="analyzing">
...
<div class="asis-section">
    <div class="asis-empty">
        <div class="asis-empty-icon">📂</div>
        <div class="asis-empty-text">분석할 파일이 없습니다</div>
        <div class="asis-empty-hint">위에서 파일을 업로드하고 파싱이 완료되면 웍스 분석을 시작할 수 있습니다.</div>
    </div>
</div>
@endif
...
<div x-show="result !== null" x-cloak>
    ...
    <div class="asis-result-section">
        ...
        <div class="asis-result-body">
            ...
            <textarea x-show="editMode" x-cloak class="asis-summary-textarea"
                      x-model="editedSummary"
                      placeholder="현황 요약을 입력하세요..."></textarea>
        </div>
    </div>
        ...
        <div class="asis-result-body">
            ...
            <div class="asis-filters">
                ...
                <template x-for="cat in uniqueCategories" :key="cat">
                    <button class="asis-filter-btn" :class="filterCategory===cat?'active':''" @click="filterCategory = filterCategory===cat ? 'all' : cat"
                            x-text="cat"></button>
                </template>
            </div>
...
{{-- ── Empty state (no result yet, no parseable files) ─────────────────── --}}
@if($files->count() === 0)
<div class="asis-section">
    <div class="asis-empty">
        <div class="asis-empty-icon">📋</div>
        <div class="asis-empty-text">아직 업로드된 파일이 없습니다</div>
        <div class="asis-empty-hint">AS-IS 현황 자료 (현행 프로세스 문서, 화면 캡처, 시스템 명세 등)를 업로드해주세요.</div>
    </div>
</div>
@endif
