...
<div onclick="pdaOpenEdit({{ $it->id }})"
     style="background:#fff;border-radius:14px;border:1px solid var(--color-bg-muted);padding:16px 20px;cursor:pointer;transition:box-shadow .12s,border-color .12s;"
        ...
        <div style="flex:1;min-width:0;">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:3px;">
                @if(!empty($showProject) && $it->project)
                    <span style="font-size:10.5px;font-weight:700;color:var(--t600);background:var(--t50);border-radius:6px;padding:2px 7px;">{{ $it->project->name }}</span>
                @endif
                @if($it->source_file_comment_id)
                    <span style="font-size:10.5px;font-weight:600;color:var(--color-text-secondary);background:var(--color-bg-muted);border-radius:6px;padding:2px 7px;display:inline-flex;align-items:center;gap:4px;">
            ...
            <div style="font-size:14px;font-weight:700;color:var(--color-text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $it->title }}</div>
        </div>
    ...
    <div style="margin-top:9px;font-size:11px;color:var(--color-text-tertiary);display:flex;gap:8px;flex-wrap:wrap;">
