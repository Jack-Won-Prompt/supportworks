...
- **`Bash`는 `permission_mode`와 무관하게 자동 승인 대상이 아니다.**
  `acceptEdits`에서 자동 승인되는 것은 `Read`/`Edit`/`Write`/`Glob`/`Grep`뿐이다.
  임의 명령 실행까지 자동 승인되면 승인 카드라는 방어선이 통째로 사라진다.
  서버(`ToolPolicy`)와 데몬(`PermissionGate`)이 같은 목록을 각자 들고 있고,
  양쪽 모두 테스트로 고정돼 있다.
...
1. **승인된 명령 안의 임의 코드.** 사용자가 `npm test`를 허용하면 그 테스트 스크립트가
   무엇이든 실행된다. 샌드박스는 명령 문자열을 보지, 그 명령이 실행할 코드를 보지 않는다.
...
| 작업이 좀비로 남는다 | 스케줄러 | `aiw:reap-stale-jobs`가 도는지. 5.4절 참조 |
...
고정해 둔 것: 상태 전이표, 툴 정책(Bash 자동 승인 금지), 채널 인가(403 회귀),
