...
| 작업 브랜치 이탈 | `git checkout main`, `git switch <branch>` (파일 단위 `git checkout -- path` 는 허용) |
...
**브랜치 이탈을 막는 이유는 보안이 아니라 흐름이다.** 반영·배포는 작업 결과가
`aiw/job-{id}` 에 있다고 보고 움직인다. AI 가 `git checkout main` 으로 나가서 기본
브랜치에 직접 커밋하면 작업 브랜치는 비어 있고, 화면의 결과 반영 버튼으로는 올릴
수 없는 상태가 된다. 실제로 그렇게 됐고(2026-09-13, Mango Shop #57), 그때 AI 는
막힌 김에 **사람에게 `git push origin main` 을 대신 실행해 달라고 했다** — 자동으로
끝까지 가야 할 일이 사람 손으로 되돌아온 것이다.

그래서 지시문 고정 헤더도 함께 고쳤다. 금지만 적으면 모델이 빈칸을 "사람이 대신
해 달라" 로 채운다. 무엇이 대신 일어나는지까지 적는다 — **사람은 SupportWorks 화면의
버튼으로 커밋·머지·푸시·배포를 하고, 모델은 준비됐다고 한 줄 알린 뒤 멈춘다.**
...
### 5.5 사람이 없어도 끝까지 가게 하는 것들

`배포까지 자동으로` 를 켜면 **지시 한 줄 → 작업 → 커밋·푸시 → 운영 배포** 가 사람 손
없이 이어진다(`AutoPipeline`). 그 길이 끊기는 자리마다 아래가 받친다.

| 무엇이 막히나 | 어떻게 푸나 | 어디 |
|---|---|---|
| 작업 폴더가 정리되지 않음 | 데몬이 보관 브랜치로 치우고 **한 번** 다시 시작 | `job-manager.ts` |
| 사람 답을 기다리다 침묵 | 10분 간격 **3회 재알림** 뒤 그침 | `aiw:nudge-waiting` |
| 담당자 PC 가 사라져 끊김 | 같은 지시·설정·첨부로 **1회 자동 재시도** | `FailureRecovery` |
| 운영 서버가 이상함 | 관리자가 등록한 **운영 명령** 실행 | 5.6절 |

자동으로 풀지 **않는** 것: 비용·시간 상한, 경로·브랜치·매핑 오류, 모델이 만든 실패
(테스트 깨짐 등). 그대로 다시 보내면 같은 자리에서 또 멈추거나, 무엇을 고칠지는
사람이 정해야 하는 것들이다. 이때는 화면의 복구 버튼과 알림으로 사람에게 넘긴다.

### 5.6 운영 명령 — 서버에 손대는 유일한 통로

배포 대상과 같은 표(`aiw_deploy_targets`)에 `kind` 로 나눠 등록한다. 실행 경로·기록·
권한은 배포와 같고, 다른 것은 **무엇을 위한 명령인가** 뿐이다.

| | 배포 | 운영 명령 |
|---|---|---|
| 쓰임 | 올린 코드를 운영에 반영 | 상태 점검·캐시 재생성·큐 재시작·롤백 |
| 확인 문구 | **받는다**(대상 이름 입력) | 받지 않는다 |
| 화면 | 푸시 성공 뒤에만 | 항상 |

실행하는 주체는 셋이다.

1. **사람** — 작업 화면의 `운영 명령` 카드 버튼.
2. **서버** — 자동 배포가 실패하면 `점검`(`AIW_HEALTH_CHECK_NAME`) 이라는 이름의
   운영 명령을 자동으로 돌려 결과를 남긴다. 사람이 화면을 열었을 때 무엇이 깨졌는지
   이미 적혀 있게 하기 위해서다. 사람이 직접 누른 배포의 실패에는 돌지 않는다.
3. **담당자(모델)** — 답변에 아래 블록을 적으면 데몬이 서버에 전달한다.
...
**이름만 오간다.** 명령 문자열은 관리자가 등록한 행에서만 오고 요청에서 오지
않는다 — 그것이 이 기능의 안전장치 전부다. 없는 이름이면 실행하지 않고 등록된
목록을 돌려주므로, 모델이 이름을 지어내고 왜 안 되는지 모르는 채 헤매지 않는다.
`ops` 종류만 찾으므로 **배포를 이 경로로 우회할 수 없다** — 확인 문구가 의미를
잃지 않는다.

되돌릴 수 없는 명령(삭제·초기화)은 운영 명령이 아니라 **배포 종류로 등록**한다.
그러면 지금과 같은 확인 문구를 받는다.

### 5.7 폴더가 준비되지 않았을 때 — 다시 점검 / 작업 정리

작업 지시 화면은 담당자 PC 가 **마지막으로 보고한** 폴더 상태를 보여 준다
(`aiw_agent_projects.setup_status`). 그래서 사람이 폴더를 정리해도 다음 보고가 올
때까지 경고가 그대로 남는다 — "고쳤는데 왜 아직 막혀 있지" 가 된다.

경고 패널의 버튼 두 개가 그 간극을 메운다. 둘 다 **관리자만** 누를 수 있고,
담당자 PC 가 온라인일 때만 보인다(요청이 실시간 채널로만 가기 때문이다).

| 버튼 | 하는 일 | 언제 보이나 |
|---|---|---|
| **다시 점검** | 데몬이 지금 상태를 다시 보고한다. 폴더를 건드리지 않는다 | 준비되지 않은 모든 상태 |
| **작업 정리** | 미커밋 변경을 `aiw/wip-<시각>` 브랜치에 커밋으로 옮기고 폴더를 정리한다 | `dirty_tree` 일 때만 |

흐름: 버튼 → `MappingSetupRequested` 브로드캐스트 → 데몬(`SetupReporter`) →
`workspace.ts` 가 점검·정리 → `POST /api/aiw/mappings/setup` 으로 보고 → 화면이
`mappings/status` 를 폴링하다가 점검 시각이 바뀌면 새로고침한다.

**작업 정리는 버리지 않는다.** `reset --hard` 도 `clean` 도 쓰지 않고, 추적되지
않던 파일까지 보관 브랜치에 담는다. 화면의 파일 목록만 보고는 그것이 누구의
작업인지 알 수 없기 때문이다 — 실제로 "미정리 변경 6건" 이 중단된 작업이 만들던
기능이었던 적이 있다(2026-09-13, Mango Shop #56).

판정 기준은 `tools/aiw-agent/src/workspace.ts` 와 `check-setup.ps1` 두 곳에 있다.
전자는 화면에서 부르는 길, 후자는 데몬이 뜰 때 한 번 도는 길이다. **문구까지 같게
유지한다** — 다르면 같은 폴더가 경로에 따라 다르게 보인다.
...
| `aiw:reap-stale-jobs` | 매분 | 데몬이 사라진 job을 실패 처리(자동 재시도로 이어짐) |
| `aiw:nudge-waiting` | 매분 | 회신·승인을 오래 기다리는 작업을 다시 알림 |
...
| 작업이 좀비로 남는다 | 스케줄러 | `aiw:reap-stale-jobs`가 도는지. 5.8절 참조 |
