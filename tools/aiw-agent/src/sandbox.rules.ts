...
export const BLOCK_RULES: BlockRule[] = [
    ...
    // ── 운영 데이터베이스 ───────────────────────────────────────────────────
    //
    // 작업 폴더의 .env 가 운영 DB 를 보고 있던 시절이 있었다. 그 상태에서 지시가
    // migrate 나 seeder 를 돌리면 사람이 없는 새벽에 운영 데이터가 바뀐다.
    // .env 는 127.0.0.1 로 돌려 두었지만, 그것은 파일 하나라 언제든 되돌아갈 수
    // 있다. 여기서 한 겹 더 막는다.
    //
    // 이 규칙이 모든 경로를 막지는 못한다 — .env 가 다시 운영을 가리키면
    // `php artisan migrate` 는 명령 문자열만 보고는 구분할 수 없다. 그래서 1차
    // 방어는 어디까지나 .env 이고, 이건 직접 접속과 주소 되돌리기를 잡는 보조다.
    {
        id: 'remote-db-client',
        // mysql/psql 류를 로컬이 아닌 호스트로 부르는 경우. -h127.0.0.1 처럼 붙여
        // 쓴 형태까지 본다. 로컬은 그대로 통과한다 — 작업에 필요하다.
        pattern: /\b(?:mysql|mysqldump|mysqladmin|mariadb|mariadb-dump|psql|pg_dump)\b[^|;&]*?\s-(?:h\s*|-host[= ])(?!(?:localhost|127\.0\.0\.1|::1)(?:\s|$))\S/i,
        reason: '원격 데이터베이스에 직접 접속할 수 없습니다. 작업 폴더의 DB 는 로컬(127.0.0.1)이며, '
            + '운영 DB 변경은 배포 스크립트를 통해서만 이루어집니다.',
    },
    {
        id: 'production-host',
        // 운영 DB 서버와 운영 웹 서버의 주소. 명령 어디에 적히든 막는다 —
        // .env 를 sed 로 되돌리는 것도 여기 걸린다.
        pattern: /(?:^|[^\d.])(?:3\.34\.53\.36|15\.165\.77\.147|43\.203\.246\.90)(?![\d.])/,
        reason: '운영 서버 주소에는 접근할 수 없습니다. 운영 반영은 화면의 배포 버튼으로만 합니다.',
    },
];
